...
if (import.meta.main) {
        ...
        for (const sub of chapter.subChapters) {
            ...
            const chunks = sentences.reduce(
                (chunks, sentence) => {
                    ...
                    const n_chunk_length = n_chunk.reduce(
                        (length, text) => length + text.length,
                        0
                    );

                    const over_characters =
                        n_chunk_length + sentence.length >= chunkMaxChars;
    ...
    for (const d of to_process) {
        for (const sentence of d.sentences) {
            ...
            const second_ai_response = await askAI<string>(
                openaiClient,
                config,
                [
                    {
                    ...
                    {
                        role: "user",
                        content: `Entities: ${first_entities
                            .map((it) => "- " + it)
                            .join("\n")}\n--\n${sentence}`,
                    },
                ]
            ...
            let triplets;
            try {
                triplets = await JSON.parse(second_ai_response);
            } catch {}
